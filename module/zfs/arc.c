...
/*
 * l2arc_buffer_type : A ZFS module parameter that controls whether the type of
 * 		buffers cached in L2ARC: MFU/MRU and metadata/data. This
 * 		reduces the wear of L2ARC devices and may be beneficial in
 * 		certain workloads.
 * 		1 --> MFU metadata
 * 		2 --> MRU metadata
 * 		4 --> MFU data
...
l2arc_write_buffers(spa_t *spa, l2arc_dev_t *dev, uint64_t target_sz)
{
	...
	for (int try = 0; try < L2ARC_FEED_TYPES; try++) {
		...
		if ((1 << try) & l2arc_buffer_type) {
			mls = l2arc_sublist_lock(try);
			VERIFY3P(mls, !=, NULL);
		} else {
			continue;
		}
...
ZFS_MODULE_PARAM(zfs_l2arc, l2arc_, buffer_type, INT, ZMOD_RW,
	"Which buffer types to cache into L2ARC, MFU/MRU, metadata/data");
